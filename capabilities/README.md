...
| 2/3 | `assignment/` | Built directly for ITAP (assignment lifecycle + rule-guarded transitions + goal setting + closure scoring + reverse feedback + administrative closure paths + optimistic concurrency + a `kind` field distinguishing Primary/Secondary/CCA per the associate-journey redesign), 67 tests passing across in-memory + SQL adapters. Not yet generalized — see note below. |
...
| — | `catalog/` | Built — Setup/Configuration data for the associate-journey redesign (`docs/associate_journey_redesign.md`): Skills/Teams/CCA-activity catalogs (admin-maintained master data) plus the associate-declared records that go with them (self-declared/engagement-sourced skills, lean profile, standing interest flags, informational annual leave). Never imports `assignment` or `party_identity` — see `docs/architecture.md`'s "Catalog capability" section for why this is its own small block rather than folded into either. 69 tests passing across in-memory + SQL adapters. |
