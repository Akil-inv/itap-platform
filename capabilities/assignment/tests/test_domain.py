...
from assignment.domain import Assignment, AssignmentKind, ClosureRecord
...
def test_assignment_kind_defaults_to_primary_for_backward_compatibility():
    assignment = Assignment(agent_id=uuid4(), manager_id=uuid4(), start_date=date(2026, 1, 1))

    assert assignment.kind == AssignmentKind.PRIMARY


@pytest.mark.parametrize("kind", [AssignmentKind.PRIMARY, AssignmentKind.SECONDARY, AssignmentKind.CCA])
def test_assignment_accepts_every_kind(kind):
    assignment = Assignment(
        agent_id=uuid4(), manager_id=uuid4(), start_date=date(2026, 1, 1), kind=kind
    )
